# ================================
# ORB SMART SCANNER PRO — v3 ENHANCED
# All enhancements applied:
#   CRITICAL: TTL cache, exponential backoff+jitter, signal deduplication
#   HIGH: 3-timeframe confluence, multiple SL strategies, multi-channel alerts
#   MEDIUM: Walk-forward BT, Monte Carlo, correlation filter, market regime
#   LOW: Session persistence, CSV watchlist import, inline candlestick, dark mode
# ================================
...
def init_state():
    defaults = {
        ...
        "trade_notes": {},
        # CRITICAL: signal deduplication — tracks active breakout state
        "signal_state": {},   # {symbol: {"direction": ..., "level": ..., "fired": bool}}
...
def load_session():
    ...
    try:
        with open(PERSISTENCE_FILE) as f:
            data = json.load(f)
        # Only restore if session is fresh (no trades loaded yet)
...
with st.sidebar:
    ...
    sl_strategy = st.selectbox(
        ...
        help="Supertrend: dynamic band | Chandelier: highest high/lowest low - N×ATR | Fixed ATR: entry ± N×ATR | Swing: recent pivot | Fixed %: percentage-based"
    )
    ...
    with st.expander("🎮 Discord"):
        discord_webhook = st.text_input("Discord Webhook URL", type="password", placeholder="https://discord.com/api/webhooks/...")
...
def cache_invalidate_expired():
    now = time.time()
    expired = [k for k, v in st.session_state.fetch_cache.items()
               if (now - v["ts"]) > 3600]
    for k in expired:
...
# ================================
# CRITICAL: ROBUST FETCH WITH BACKOFF + JITTER
# ================================
...
def fetch(symbol: str, interval: str = "5m", period: str = "5d", use_cache: bool = True) -> tuple:
    """
    Fetch OHLCV with TTL cache, exponential backoff + jitter on retry.
    Returns (symbol, df, error_str_or_None)
    """
...
def fetch_nifty_vix():
    ...
    try:
        _, nifty, _ = fetch("^NSEI", interval="1d", period="5d", use_cache=False)
        ...
        pass
    try:
        _, vix, _ = fetch("^INDIAVIX", interval="1d", period="5d", use_cache=False)
...
# ================================
# MEDIUM: MARKET REGIME DETECTOR
# ================================
def detect_market_regime(df: pd.DataFrame) -> str:
    """
    Classify as Trending / Ranging / Volatile using ADX + Bollinger Band Width.
    """
    ...
    try:
        ...
        dm_minus = np.where(pd.Series(dm_minus) > pd.Series(dm_plus), dm_minus, 0)

        smooth_plus  = pd.Series(dm_plus).rolling(14, min_periods=1).mean()
        smooth_minus = pd.Series(dm_minus).rolling(14, min_periods=1).mean()
...
# ================================
# HIGH: MULTIPLE SL STRATEGIES
# ================================
def compute_sl(df: pd.DataFrame, direction: str, entry_price: float, orb_low: float, orb_high: float) -> float:
    """
    Compute stop-loss using the selected SL strategy.
    """
    ...
    if sl_strategy == "Supertrend":
        ...
        if direction == "BUY":
            highest_high = float(df["High"].rolling(n, min_periods=1).max().iloc[-1])
            return round(highest_high - sl_atr_mult * atr, 2)
        else:
            lowest_low = float(df["Low"].rolling(n, min_periods=1).min().iloc[-1])
            return round(lowest_low + sl_atr_mult * atr, 2)

    elif sl_strategy == "Fixed ATR":
        if direction == "BUY":
            return round(entry_price - sl_atr_mult * atr, 2)
        else:
            return round(entry_price + sl_atr_mult * atr, 2)

    elif sl_strategy == "Swing High/Low":
        if direction == "BUY":
            swing_low = float(df["Low"].rolling(n, min_periods=1).min().iloc[-1])
            return round(swing_low - 0.1 * atr, 2)
        else:
            swing_high = float(df["High"].rolling(n, min_periods=1).max().iloc[-1])
            return round(swing_high + 0.1 * atr, 2)

    elif sl_strategy == "Fixed %":
        if direction == "BUY":
            return round(entry_price * (1 - trail_pct), 2)
        else:
            return round(entry_price * (1 + trail_pct), 2)
...
def update_sl(df: pd.DataFrame, trade: dict, current_price: float) -> float:
    """Update trailing SL based on selected strategy."""
    ...
    if direction == "BUY":
        ...
        return round(min(current_sl, new_sl), 2)
...
def is_new_breakout(symbol: str, direction: str, level: float) -> bool:
    """
    Return True only if this is a NEW breakout event.
    Prevents same signal firing every scan cycle.
    """
    ...
    # Same direction+level but already fired
    if prev_fired:
        return False
...
def reset_signal_state(symbol: str):
    """Called when price returns inside ORB — ready for new breakout."""
    if symbol in st.session_state.signal_state:
        st.session_state.signal_state[symbol]["fired"] = False


# ================================
# MEDIUM: CORRELATION FILTER
# ================================
def is_correlated_with_active(symbol: str, df: pd.DataFrame) -> bool:
    """
    Returns True if this symbol is highly correlated (r > 0.8) with
    an already-active trade IN THE SAME SECTOR.
    """
    ...
    try:
        ...
        for active_sym in active_same_sector:
            cached = cache_get(active_sym, "5m")
            if cached is None:
                continue
            close_other = cached["Close"].dropna()
...
def get_signal(symbol: str, df_5m: pd.DataFrame, df_15m=None, df_1h=None):
    ...
    if not breakout_confirmed(df, level, direction):
        # Dedup: don't mark as fired yet if breakout not confirmed
        st.session_state.signal_state[symbol]["fired"] = False
    ...
    # Market regime filter (MEDIUM)
    if use_regime:
        regime = detect_market_regime(df)
        st.session_state.market_regime = regime
        if regime == "Ranging":
            st.session_state.signal_state[symbol]["fired"] = False
            return None, f"Market regime is Ranging — ORB suppressed"

    # HIGH: 3-Timeframe Confluence — 1h EMA
    tf3_ok = None
    if use_3tf and df_1h is not None and len(df_1h) >= ema_1h_period + 2:
        df1h = add_indicators_1h(df_1h)
        ema_now  = float(df1h["EMA1H"].iloc[-1])
        ema_prev = float(df1h["EMA1H"].iloc[-2])
        tf3_rising = ema_now > ema_prev
        tf3_ok = tf3_rising
    ...
    if use_partial and not trade["partial_done"] and trade["qty_remaining"] > 1:
        ...
        if hit_partial:
            ...
            if partial_qty > 0:
                ...
                msg = f"📦 Partial Exit — {symbol}\n{partial_qty} qty @ ₹{price:.2f} | P&L: ₹{partial_pnl:+.2f}\nSL → breakeven ₹{trade['entry']}"
                send_alert(msg, f"📦 <b>Partial Exit — {symbol}</b>\n{partial_qty} qty @ ₹{price:.2f} | P&L: ₹{partial_pnl:+.2f}")

    # Trailing SL update using selected strategy
...
def exit_trade(symbol, price, pnl, reason="Manual"):
    ...
    emoji = "✅" if pnl > 0 else "❌"
    msg = f"{emoji} Trade Exit — {symbol}\nExit @ ₹{price:.2f} | P&L: ₹{pnl:+.2f}\nReason: {reason}"
    html_msg = f"{emoji} <b>Trade Exit — {symbol}</b>\nExit @ ₹{price:.2f} | P&L: ₹{pnl:+.2f}\nReason: {reason}"
    send_alert(msg, html_msg)
    # Reset signal state on exit so next breakout can fire
    reset_signal_state(symbol)
...
# MAIN SCAN
# ================================
run_scan = (market_open or st.session_state.get("_manual_scan")) and not st.session_state.trading_locked

# Manual scan button handled below in header section
...
with col_h1:
    st.markdown('<div class="header-title">🚀 ORB Smart Scanner <span style="color:#6366f1;font-size:1rem;font-weight:600;vertical-align:middle;">PRO v3</span></div>', unsafe_allow_html=True)
    status_txt = "🟢 Market Open" if market_open else ("🟡 Pre-Market" if is_pre_market() else "🔴 Market Closed")
    regime_cls = {"Trending": "regime-badge-trend", "Ranging": "regime-badge-range", "Volatile": "regime-badge-volatile"}.get(st.session_state.market_regime, "regime-badge-range")
    st.markdown(
...
if do_scan:
    ...
    for i in range(0, total, BATCH):
        ...
        with ThreadPoolExecutor(max_workers=min(10, len(batch))) as ex:
            ...
            if use_3tf:
                futures.update({ex.submit(fetch, s, "1h", "5d"): ("1h", s) for s in batch})
    ...
    for symbol, df, err in all_data:
        ...
        if n_active >= max_trades:
            ...
            # Revert dedup so it can fire when slot opens
            st.session_state.signal_state[symbol]["fired"] = False
            continue
...
regime = st.session_state.market_regime
regime_cls = {"Trending": "regime-badge-trend", "Ranging": "regime-badge-range", "Volatile": "regime-badge-volatile"}.get(regime, "regime-badge-range")
    ...
    if st.session_state.scan_results:
        ...
        if chart_sym_opts:
            ...
            if st.button("📊 Load Chart", key="load_chart_btn"):
                _, df_chart, _ = fetch(chart_sym, "5m", "5d", use_cache=True)
                if df_chart is not None:
                    ...
                    if "ST_val" in df_chart.columns:
                        colors = ["#16a34a" if d == 1 else "#dc2626" for d in df_chart["ST_dir"]]
                        fig_c.add_trace(go.Scatter(
...
with tab6:
    ...
    if st.session_state.scan_results:
        ...
        if top_sector is not None and top_sector["Total Signals"] > 3:
            st.warning(f"⚠️ Over-concentration: **{top_sector['Sector']}** has {top_sector['Total Signals']} signals. {'Correlation filter is ON — duplicates suppressed.' if use_corr_filter else 'Enable correlation filter to suppress duplicates.'}")
    else:
...
with tab7:
    ...
    st.caption("True out-of-sample validation with rolling windows. Each window trains on N days and tests on the next M days.")
    ...
    def run_orb_backtest_on_days(df_full, days_list, orb_m, atr_m):
        """Run ORB on specific days list. Returns list of trade dicts."""
        trades = []
    ...
    if st.button("▶️ Run Walk-Forward Backtest", type="primary", key="wf_run"):
        with st.spinner("Running walk-forward backtest…"):
            ...
            for sym in bt_symbols:
                _, df_bt, err = fetch(sym, "5m", "1mo", use_cache=False)
                if err or df_bt is None or len(df_bt) < 20:
                    continue
                df_bt.index = pd.to_datetime(df_bt.index)
                all_days = sorted(df_bt.index.date.tolist())
                ...
                for start_i in range(0, len(unique_days) - window_size + 1, int(wf_test_days)):
                    ...
                    test_trades = run_orb_backtest_on_days(df_bt, test_days, bt_orb_min, bt_atr_tgt)
...
with tab8:
    ...
    st.caption("Shuffles trade outcomes 1000+ times to estimate true max drawdown distribution and risk-of-ruin.")
    ...
    if st.button("▶️ Run Monte Carlo", type="primary", key="mc_run"):
        # Use closed trades if available, else backtest results
        ...
        if len(pnl_list) < 5:
            ...
            with st.spinner(f"Running {mc_n_sims:,} simulations…"):
                ...
                for _ in range(int(mc_n_sims)):
                    ...
                    ruined   = any(equity <= mc_capital * (1 - mc_risk_ruin))
                    results.append({"final": final, "max_dd": max_dd, "ruined": ruined})
                ...
                with mc1:
                    st.markdown(f'<div class="backtest-stat"><div class="val">{ruin_rate:.1f}%</div><div class="lbl">Risk of Ruin</div></div>', unsafe_allow_html=True)
                with mc2:
                    st.markdown(f'<div class="backtest-stat"><div class="val" style="color:#dc2626;">{np.percentile(dd_arr, 95):.1f}%</div><div class="lbl">95th % Max DD</div></div>', unsafe_allow_html=True)
                with mc3:
                    st.markdown(f'<div class="backtest-stat"><div class="val">₹{np.percentile(final_arr, 50):,.0f}</div><div class="lbl">Median Outcome</div></div>', unsafe_allow_html=True)
                with mc4:
                    st.markdown(f'<div class="backtest-stat"><div class="val">₹{np.percentile(final_arr, 5):,.0f}</div><div class="lbl">5th % Worst Case</div></div>', unsafe_allow_html=True)
                ...
                fig_mc1.add_trace(go.Histogram(
                    x=final_arr, nbinsx=50,
                    marker_color="#6366f1", opacity=0.8,
                    name="Final Capital"
                ))
                fig_mc1.add_vline(x=mc_capital, line_dash="dash", line_color="#94a3b8",
                                   annotation_text="Start Capital")
                fig_mc1.add_vline(x=float(np.percentile(final_arr, 5)), line_dash="dot",
                                   line_color="#ef4444", annotation_text="5th %ile")
                fig_mc1.add_vline(x=float(np.percentile(final_arr, 50)), line_dash="dot",
                ...
                fig_mc2.add_vline(x=float(np.percentile(dd_arr, 95)), line_dash="dot",
                                   line_color="#7c3aed", annotation_text="95th %ile")
                fig_mc2.update_layout(
                    title="Max Drawdown Distribution",
                    template=plotly_tpl, height=300,
                    margin=dict(l=10, r=10, t=50, b=10)
                )
...
with tab9:
    if st.session_state.error_log:
        ...
        st.info(
            "**Common causes:**\n"
            "- Market closed / weekend (5m data unavailable — scanner uses 5d period as fallback)\n"
            "- Symbol delisted or wrong suffix (.NS vs .BO)\n"
            "- Yahoo Finance rate limiting (built-in exponential backoff + jitter will retry automatically)\n"
